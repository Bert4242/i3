...
int main(int argc, char *argv[]) {
    ...
    while ((o = getopt_long(argc, argv, options_string, long_options, &option_index)) != -1) {
        if (o == 's') {
            ...
            if (strcasecmp(optarg, "command") == 0) {
            ...
            } else if (strcasecmp(optarg, "get_seats") == 0) {
                message_type = I3_IPC_MESSAGE_TYPE_GET_SEATS;
                ...
                printf("Known types: run_command, get_workspaces, get_outputs, get_tree, get_marks, get_bar_config, get_binding_modes, get_binding_state, get_seats, get_version, get_config, send_tick, subscribe\n");
                exit(EXIT_FAILURE);
            }
        } else if (o == 'q') {
            quiet = true;
