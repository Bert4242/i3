...
/*
 * Translates an XInput2 motion event's Fp1616 root coordinates into a
 * heap-allocated xcb_motion_notify_event_t, matching the shape
 * drain_drag_events() already knows how to consume (only root_x/root_y are
 * read). i3 does not need sub-pixel precision here (see click.c's
 * xinput_translate_button_event() for the same reasoning).
 *
 */
static xcb_motion_notify_event_t *xinput_translate_motion_event(const xcb_input_button_press_event_t *xi_event) {
    xcb_motion_notify_event_t *event = scalloc(1, sizeof(xcb_motion_notify_event_t));
...
static bool drain_drag_events(EV_P, struct drag_x11_cb *dragloop) {
    ...
    while ((event = xcb_poll_for_event(conn)) != NULL) {
        ...
        /* When dragging tracks a device via XInput2 (see drag_pointer()),
         * motion/button-release for that grab arrive as XInput2
         * GenericEvents rather than core XCB_MOTION_NOTIFY /
         * XCB_BUTTON_RELEASE. Translate them into the same shapes the rest
         * of this function already handles below so nothing else here
         * needs to know XInput2 is involved. */
        bool free_original_event = true;
        if (xinput_supported && type == XCB_GE_GENERIC) {
            ...
            if (generic->extension == xinput_opcode) {
                if (generic->event_type == XCB_INPUT_BUTTON_RELEASE) {
    ...
    if (!dragloop->threshold_exceeded &&
        ...
        if (dragloop->xcursor != XCB_NONE) {
            if (xinput_supported) {
                /* XInput2 has no equivalent to xcb_change_active_pointer_grab();
                 * re-grab with the new cursor instead. This is the same active
                 * grab drag_pointer() already holds on xinput_last_event_device,
                 * just re-issued with a cursor, so it does not interrupt the
                 * drag. */
                const uint32_t mask[] = {XCB_INPUT_XI_EVENT_MASK_BUTTON_RELEASE | XCB_INPUT_XI_EVENT_MASK_MOTION};
                xcb_input_xi_grab_device(conn, root, XCB_CURRENT_TIME, dragloop->xcursor,
                                         xinput_last_event_device, XCB_INPUT_GRAB_MODE_22_ASYNC,
                                         XCB_INPUT_GRAB_MODE_22_ASYNC, 0, 1, mask);
            } else {
...
drag_result_t drag_pointer(Con *con, const xcb_button_press_event_t *event,
    ...
    /* Grab the pointer.
     *
     * When XInput2 button grabs are in use (see xinput.c), we must grab the
     * pointer via XInput2 too, targeting the specific master pointer device
     * that initiated this drag (xinput_last_event_device). A plain core
     * xcb_grab_pointer() always targets the primary core pointer, which (a)
     * cannot pick up motion/release from a different master pointer (e.g. a
     * touch device) at all, and (b) does not thaw the XInput2-level freeze
     * left by our SYNC button grab, since core and XInput2 grabs are
     * separate freeze domains: the drag would silently never receive any
     * events. */
    if (xinput_supported) {
        const uint32_t mask[] = {XCB_INPUT_XI_EVENT_MASK_BUTTON_RELEASE | XCB_INPUT_XI_EVENT_MASK_MOTION};
        xcb_input_xi_grab_device_cookie_t cookie = xcb_input_xi_grab_device(
            conn, root, XCB_CURRENT_TIME, use_threshold ? XCB_NONE : xcursor,
            xinput_last_event_device, XCB_INPUT_GRAB_MODE_22_ASYNC, XCB_INPUT_GRAB_MODE_22_ASYNC,
            0, 1, mask);
        xcb_input_xi_grab_device_reply_t *reply = xcb_input_xi_grab_device_reply(conn, cookie, &error);
        if (reply == NULL) {
            ELOG("Could not grab device %d via XInput2 (error_code = %d)\n", xinput_last_event_device, error->error_code);
            free(error);
            return DRAG_ABORT;
        }
        ...
        free(reply);
        if (status != 0) {
            ELOG("Could not grab device %d via XInput2 (status = %d)\n", xinput_last_event_device, status);
            return DRAG_ABORT;
        }
    } else {
