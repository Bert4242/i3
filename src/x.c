...
void x_con_init(Con *con) {
    ...
    /* Select pointer events via XInput2 (see include/xcb.h,
     * FRAME_EVENT_MASK) so that decoration clicks carry a device id; like
     * the core mask above, without enter events until the frame is mapped. */
    xinput_select_frame_events(conn, frame_id, false);
...
void x_push_node(Con *con) {
    ...
    if ((state->mapped != con->mapped || (con->window != NULL && !state->child_mapped)) &&
        ...
        xinput_select_frame_events(conn, con->frame.id, true);
...
void x_push_changes(Con *con) {
    ...
    /* The pointer to warp: the acting seat's. */
    const xcb_input_device_id_t warp_device = seat_first_pointer(current_seat);
    const bool warp_via_xinput = (xinput_supported && warp_device != SEAT_DEVICE_NONE);
    xcb_input_xi_query_pointer_cookie_t xi_pointercookie;

    /* If we need to warp later, we request the pointer position as soon as possible */
    if (warp_to) {
        if (warp_via_xinput) {
            xi_pointercookie = xcb_input_xi_query_pointer(conn, root, warp_device);
        } else {
    ...
    CIRCLEQ_FOREACH_REVERSE (state, &state_head, state) {
        if (state->mapped) {
            ...
            xinput_select_frame_events(conn, state->id, false);
        }
    }
    ...
    if (warp_to) {
        int16_t pointer_x = 0, pointer_y = 0;
        bool have_pointer = false;
        if (warp_via_xinput) {
            xcb_input_xi_query_pointer_reply_t *reply = xcb_input_xi_query_pointer_reply(conn, xi_pointercookie, NULL);
            if (reply != NULL) {
                pointer_x = (int16_t)(reply->root_x >> 16);
                pointer_y = (int16_t)(reply->root_y >> 16);
                have_pointer = true;
                free(reply);
            }
        } else {
            xcb_query_pointer_reply_t *reply = xcb_query_pointer_reply(conn, pointercookie, NULL);
            if (reply != NULL) {
                pointer_x = reply->root_x;
                pointer_y = reply->root_y;
                have_pointer = true;
                free(reply);
            }
        }
        ...
        if (!have_pointer) {
            ...
            Output *current = get_output_containing(pointer_x, pointer_y);
            ...
            if (current != target) {
                ...
                xinput_select_root_events(conn, false);
                xinput_warp_pointer(conn, warp_device, root, mid_x, mid_y);
    ...
    CIRCLEQ_FOREACH_REVERSE (state, &state_head, state) {
        if (state->mapped) {
            xcb_change_window_attributes(conn, state->id, XCB_CW_EVENT_MASK, values);
            xinput_select_frame_events(conn, state->id, true);
        }
    }
...
void x_mask_event_mask(uint32_t mask) {
    ...
    CIRCLEQ_FOREACH_REVERSE (state, &state_head, state) {
        if (state->mapped) {
            xcb_change_window_attributes(conn, state->id, XCB_CW_EVENT_MASK, values);
            xinput_select_frame_events(conn, state->id, (mask & XCB_EVENT_MASK_ENTER_WINDOW) != 0);
        }
    }
}
